...
use num::{BigUint, Zero, One, ToPrimitive};
...
pub trait Field : Clone + Default { // To do:  Implement binary fields.
    fn factors(&self) -> (usize, Vec<isize>);
    fn modulus(&self) -> BigUint;
    fn reduce(&self, z: BigUint) -> FieldElem<Self>;
    fn unserialize(&self, s: &Vec<usize>) -> FieldElem<Self>;
}
...
impl<F: Field> Add<FieldElem<F>> for FieldElem<F> {
    type Output = FieldElem<F>;
...
impl<F: Field> Sub<FieldElem<F>> for FieldElem<F> {
    type Output = FieldElem<F>;
...
impl<F: Field> Mul<FieldElem<F>> for FieldElem<F> {
    type Output = FieldElem<F>;
...
impl<F: Field> Div<FieldElem<F>> for FieldElem<F> {
    type Output = FieldElem<F>;
...
impl<F: Field> Neg for FieldElem<F> {
    type Output = FieldElem<F>;
...
impl<F: Field> Zero for FieldElem<F> {
    fn zero() -> FieldElem<F> {
        let f: F = Default::default();
        f.reduce(0isize.to_biguint().unwrap())
    }
...
impl<F: Field> One for FieldElem<F> {
    fn one() -> FieldElem<F> {
        let f: F = Default::default();
        f.reduce(1isize.to_biguint().unwrap())
    }
}
...
impl<F: Field> FieldElem<F> {
    pub fn serialize(&self) -> Vec<u32> {
        ...
        for i in 0..m {
            out.push(((&self.limbs >> ((m - i - 1) * 8)) & &mask).to_u32().unwrap())
        }
    ...
    pub fn pow(&self, cand_exp: &BigUint) -> FieldElem<F> {
        ...
        let order: BigUint = &f.modulus() - &one;
...
impl<F: Field> fmt::Display for FieldElem<F> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.limbs)
    }
}

fn reduce(modulus: (usize, Vec<isize>), z: BigUint) -> BigUint {
    ...
    for part in reducs.iter() {
        if *part > 0 {
            out = &out - (&u << (*part as usize))
        } else {
            out = &out + (&u << ((*part * -1) as usize))
        }
    }
...
fn unserialize(s: &Vec<usize>) -> BigUint {
...
pub struct P192;
impl Field for P192 {
    fn factors(&self) -> (usize, Vec<isize>) { (192, vec![-64, 0]) }
    fn modulus(&self) -> BigUint {
        let one: BigUint = One::one();
    ...
    fn reduce(&self, z: BigUint) -> FieldElem<P192> {
        if z.bits() > 192 { self.reduce(reduce(self.factors(), z)) }
            ...
            else { FieldElem { limbs: z, f: PhantomData, } }
        } else { FieldElem { limbs: z, f: PhantomData, } }
    }

    fn unserialize(&self, s: &Vec<usize>) -> FieldElem<P192> {
...
impl Field for R192 {
    fn factors(&self) -> (usize, Vec<isize>) { (192, vec![-64, 0]) }
    fn modulus(&self) -> BigUint {
        BigUint::parse_bytes(b"6277101735386680763835789423176059013767194773182842284081", 10).unwrap()
    }

    fn reduce(&self, z: BigUint) -> FieldElem<R192> {
        FieldElem { limbs: z % self.modulus(), f: PhantomData, }
    }

    fn unserialize(&self, s: &Vec<usize>) -> FieldElem<R192> {
        self.reduce(unserialize(s))
    }
}
...
pub struct P256;
impl Field for P256 {
    fn factors(&self) -> (usize, Vec<isize>) { (256, vec![-224, 192, 96, 0]) }
    fn modulus(&self) -> BigUint {
        let one: BigUint = One::one();
    ...
    fn unserialize(&self, s: &Vec<usize>) -> FieldElem<P256> {
        self.reduce(unserialize(s))
    }
}
...
impl Field for R256 {
    fn factors(&self) -> (usize, Vec<isize>) { (256, vec![-224, 192, 96, 0]) }
    fn modulus(&self) -> BigUint {
        BigUint::parse_bytes(b"115792089210356248762697446949407573529996955224135760342422259061068512044369", 10).unwrap()
    }
    ...
    fn unserialize(&self, s: &Vec<usize>) -> FieldElem<R256> {
        self.reduce(unserialize(s))
    }
}
...
pub struct P521;
impl Field for P521 {
    fn factors(&self) -> (usize, Vec<isize>) { (521, vec![0]) }
    fn modulus(&self) -> BigUint {
        let one: BigUint = One::one();
    ...
    fn unserialize(&self, s: &Vec<usize>) -> FieldElem<P521> {
        self.reduce(unserialize(s))
    }
}
...
impl Field for R521 {
    fn factors(&self) -> (usize, Vec<isize>) { (521, vec![0]) }
    fn modulus(&self) -> BigUint {
        BigUint::parse_bytes(b"6864797660130609714981900799081393217269435300143305409394463459185543183397655394245057746333217197532963996371363321113864768612440380340372808892707005449", 10).unwrap()
    }
    ...
    fn unserialize(&self, s: &Vec<usize>) -> FieldElem<R521> {
        self.reduce(unserialize(s))
    }
}
...
mod tests {
    ...
    fn large_exponentiation() {
        let p: FieldElem<P192> = FieldElem { limbs: 3isize.to_biguint().unwrap(), f: PhantomData };

        let x = p.pow(&(300isize.to_biguint().unwrap())).limbs;
        let y: Option<BigUint> = BigUint::parse_bytes(b"301636b368b805e333093013fb588bb4f3bc9fd0578b2e6", 16);
    ...
    fn bench_exponentiation_p192(b: &mut Bencher) {
        let p: FieldElem<P192> = FieldElem {
            limbs: BigUint::parse_bytes(b"7e48c5ab7f43e4d9c17bd9712627dcc76d4df2099af7c8e5", 16).unwrap(),
            f: PhantomData
        };
        let exp = 3000isize.to_biguint().unwrap();
    ...
    fn bench_exponentiation_p192_right_zeroes(b: &mut Bencher) {
        let p: FieldElem<P192> = FieldElem {
            limbs: BigUint::parse_bytes(b"7e0000000000000000000000000000000000000000000000", 16).unwrap(),
            f: PhantomData
        };
        let exp = 3000isize.to_biguint().unwrap();
